	...
	while(gameCont) { //while the game is not over
		if(turn==compColour) { //if its the computers turn
			...
			if(checkForMoves(board, dimension, userColour)) { //checks if the user has avail moves
				...
				scanf("%c%c%c", &garb, &rowc, &colc);
				
				row=rowc-'a';
				col=colc-'a';
				if(checkValid(board, dimension, row, col, userColour)) { //must be valid position
					checkScoreAndFlip(board, dimension, row, col, userColour, true); //flips tiles
					printBoard(board, dimension);
				} 
